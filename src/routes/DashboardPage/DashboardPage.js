...
export default class DashboardPage extends React.Component {
    ...
    getMinutes(item) {
        ...
        if (item.difference.days) {
            minutes += (item.difference.days*24*60 + item.difference.hours*60 + item.difference.minutes)
        } else if (!item.difference.days && item.difference.hours) {
            minutes += (item.difference.hours*60 + item.difference.minutes)
        } else if (!item.difference.hours && item.difference.minutes) {
            minutes += item.difference.minutes
        } else if (!item.difference.minutes && item.difference.seconds) {
            minutes += 1
        }
